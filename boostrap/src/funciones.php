		...
		case 'recupera-geojson-cluster-user':
			{
				...
				FROM ( SELECT 'FeatureCollection' As type, array_to_json(array_agg(f)) As features
				FROM (SELECT 'Feature' As type, ST_AsGeoJSON(lg.geom)::json As geometry, row_to_json
				((SELECT l FROM (SELECT  lg.tipo, lg.barrio, lg.descripcion, lg.id_reporte, lg.fecha_registro, lg.usuario  ) As l)) As properties
				FROM (SELECT  st_setsrid(st_makepoint(r.x,r.y),4326) as geom ,  b.barrio, r.tipo, r.descripcion, r.id_reporte, r.fecha_registro, r.usuario, r.id_usuario 
			FROM reporte as r, barrios as b WHERE st_within(st_setsrid(st_makepoint(r.x,r.y),4326), b.geom ) and r.usuario  ='$user'  
		   	) As lg   
			) As f )  As fc;";
